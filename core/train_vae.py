...
def freeze_model(sess, logs_path, latest_checkpoint, model):
    print("logs_path =", logs_path)
    tf.train.write_graph(sess.graph.as_graph_def(), logs_path, 'encoder_train.pb')
    input_graph_path = os.path.join(logs_path, 'encoder_train.pb')
    input_saver_def_path = ""
    input_binary = False
    input_checkpoint_path = latest_checkpoint
    output_graph_path = os.path.join(logs_path, cs.ENCODER_FREEZED_PB_NAME)
    clear_devices = False
...
def train():
    loading = False
    logs_path = cs.BASE_LOG_PATH + cs.SLASH + cs.MODEL_VAE
    ...
    utility.freeze_model(sess, logs_path, tf.train.latest_checkpoint(logs_path),
                         vae, "encoder_train.pb", cs.ENCODER_FREEZED_PB_NAME)
